...
class AppDelegate: NSObject, NSApplicationDelegate, HashiCentralDelegate {
    ...
    func didUpdateAcceleration(accX: Float, accY: Float, accZ: Float) {
        var x = NSString(format: "%.3f g", accX)
        var y = NSString(format: "%.3f g", accY)
        var z = NSString(format: "%.3f g", accZ)
        ...
        if(recordFlag == true){
            // 実際の保存
            var fileObj:NSString = ""
            fileObj = fileObj + x + "," + y + "," + z + "¥n"
            // fileObj.writeToFile(<#path: String#>, atomically: <#Bool#>, encoding: <#NSStringEncoding#>, error: <#NSErrorPointer#>)
            fileObj.writeToFile("/User/shack/Desktop/Hashi/hashi.log", atomically: false, encoding: NSUTF8StringEncoding, error: nil)
